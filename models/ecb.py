...
class ECB_sobel(nn.Module):
    def __init__(self, dim, depth_multiplier=2, act_type='linear', with_idt=False, permute=True, **kwargs):
        super(ECB_sobel, self).__init__()
...
class ECB_lap(nn.Module):
    def __init__(self, dim, depth_multiplier=2, act_type='linear', with_idt=False, permute=True, **kwargs):
        super(ECB_lap, self).__init__()
...
if __name__ == '__main__':
    torch.manual_seed(1)
    # # test seq-conv
    # x = torch.randn(1, 3, 5, 5, 5).cuda()
    # conv = SeqConv3x3x3('conv1x1x1-conv3x3x3', 3, 3, 2).cuda()
    # y0 = conv(x)
    # RK, RB = conv.rep_params()
    # y1 = F.conv3d(input=x, weight=RK, bias=RB, stride=1, padding=1)
    # print(y0-y1)
    
#     conv = SeqConv3x3x3('conv1x1x1-sobelz', 3, 3, 2).cuda()
#     y0 = conv(x)
#     RK, RB = conv.rep_params()
#     y1 = F.conv3d(input=x, weight=RK, bias=RB, stride=1, padding=1)
#     print(y0-y1)
...
#     # test ecb
    x = torch.randn(1, 3, 5, 5, 5).cuda() * 200
    ecb = ECB(3, 3, 2, act_type='prelu', with_idt=False).cuda()
    y0 = ecb(x)

    RK, RB = ecb.rep_params()
    y1 = F.conv3d(input=x, weight=RK, bias=RB, stride=1, padding=1)
    act = nn.PReLU(num_parameters=3).cuda()
    y1 = act(y1)
    print(y0-y1)